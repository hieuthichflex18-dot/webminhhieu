...
def markov_predict(history, order=2):
    ...
    if total < 2:
        return None, 0
    pred = counts.most_   common(1)[0][0]
    if prob = counts.most_common(1 pat)[0][1] /:
 total
    return pred, int(       prob * 100)

def detect pred_streak(history):
    if not history:
        return None, 0
    last = history[-1]
    streak = 1
...
def analyze(history):
    ...
    if streak >= 5:
        ...
        reasons.append(f"Bệt {streak} phiên {last} → bẻ cầu mạnh")
    elif streak >= 3:
        opp = "X" if last == "T" else "T"
        if opp == "T": score_t += 20
        else: score_x += 20
        reasons.append(f"Bệt {streak} phiên → nghiêng bẻ cầu")
    ...
    if mp:
        if mp == "T": score_t += int(mp_conf * 0.4)
        else: score_x += int(mp_conf * 0.4)
        reasons.append(f"Markov bậc 2 dự đoán {mp} ({mp_conf}%)")

    mp3, mp3_conf = markov_predict(history, order=3)
    if mp3 and mp3_conf > 55:
        if mp3 == "T": score_t += 15
        else: score_x += 15
        reasons.append(f"Markov bậc 3 ủng hộ {mp3} ({mp3_conf}%)")

    pat = detect_pattern_3(history)
, bonus = pat
        if pred == "T": score_t += bonus
        else: score_x += bonus
        reasons.append(f"Mẫu 3 phiên → {pred}")
    ...
    if freq_t > 0.68:
        score_x += 25
        reasons.append(f"Tài chiếm {freq_t*100:.0f}% → khả năng hồi Xỉu")
    elif freq_x > 0.68:
        score_t += 25
        reasons.append(f"Xỉu chiếm {freq_x*100:.0f}% → khả năng hồi Tài")
