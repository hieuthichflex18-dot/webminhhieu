    ...
    if request.method == "POST":
            ...
            try:
                if bcrypt.checkpw(p.encode(), ADMIN_PASS_HASH.encode()):
                    session.update({
                        ...
                        "tier": "admin", "login_at": datetime.now().isoformat(),
                    })
        ...
        with get_db() as db:
            ...
            if row and bcrypt.checkpw(p.encode(), row["password_hash"].encode()):
                ...
                session.update({
                    "username": u, "role": row["role"],
                    "tier": row["tier"], "login_at": datetime.now().isoformat(),
                })
...
def api_game_detail(game_key):
    ...
    now = datetime.now()
    session_id = f"#{game_key}-{now.strftime('%H%M%S')}"
    ...
    return jsonify({
        ...
        "block_prediction": {
            "title": f"DỰ ĐOÁN PHIÊN SAU · {game['name']}",
            "mode": game["mode"],
...
def api_redeem():
    ...
    if ok:
        with get_db() as db:
            row = db.execute("SELECT tier FROM users WHERE username=?", (session["username"],)).fetchone()
            if row:
...
def admin_stats():
    ...
    return jsonify({
        "total_users": users, "total_keys": keys, "active_keys": active_keys,
        "total_predictions": preds, "correct_predictions": correct,
        "accuracy": round(correct / preds * 100, 1) if preds else 0,
    })
...
def admin_users():
    with get_db() as db:
        rows = db.execute("SELECT id, username, role, tier, expires_at, created_at, last_login, active FROM users ORDER BY id DESC").fetchall()
...
def admin_create_user():
    ...
    try:
        with get_db() as db:
            db.execute("""
                INSERT INTO users (username, password_hash, role, tier, expires_at, created_by)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (u, pw_hash, role, tier, exp, session["username"]))
        log_action(session["username"], f"create_user:{u}", request.remote_addr, "")
        return jsonify({"ok": True, "message": f"Đã tạo user {u}"})
    except Exception as e:
        return jsonify({"ok": False, "message": f"Lỗi: {e}"})
