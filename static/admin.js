...
async function loadUsers() {
  ...
  tbody.innerHTML = users.map(u => `
    <tr>
      ...
      <td style="font-size:0.75rem;">${u.last_login ? u.last_login.substring(0,',' ') : '—'}</td>
      <td>${u.active ? '✅' : '🚫'}</td>
      <td>
...
async function createKey() {
  ...
  document.getElementById('keyDisplay').innerHTML =
    `<div class="key-display" onclick="navigator.clipboard.writeText('${d.key}');this.textContent='✅ Đã copy: ${d.key}';">
      ${d.key} <span style="font-size:0.75rem;opacity:0.7;margin-left:8px;">(click copy)</span>
    </div>`;
  loadKeys(); loadStats();
}
